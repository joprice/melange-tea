  ...
  Expect
    ...
    , fun {body; _} ->
        ...
        | TextResponse s ->
            func s
        | _ ->
            Tea_result.Error "Non-text response returned" )
...
let encodeUri str = Js.Global.encodeURIComponent str

let decodeUri str = try Some (Js.Global.decodeURIComponent str) with _ -> None
