...
type 'model debug_model =
  {history: (string * 'model) list; state: state; show_details: bool}
...
let debug (string_of_msg : 'msg -> string)
  ...
  let update' model = function
    | ClientMsg msg ->
        if model.state = Running then
          ...
          let dmodel' =
            {model with history= (string_of_msg msg, cmodel') :: model.history}
          in
    ...
    | TogglePaused -> (
      ...
      | Paused _ ->
          ({model with state= Running}, Tea_cmd.none)
      | Running ->
          ({model with state= Paused 0}, Tea_cmd.none) )
    | SelectHistoryItem i ->
  ...
  let view' model =
    ...
    let selected_index, selected_model, paused =
      match model.state with
      | Running ->
          (0, List.hd model.history |> snd, false)
      | Paused index ->
          (index, List.nth model.history index |> snd, true)
    in
...
let standardProgram :
 ...
 fun {init; update; view; subscriptions} string_of_msg pnode flags ->
  let debugged =
    debug_program string_of_msg
      {init; update; view; subscriptions; shutdown= (fun _model -> Tea_cmd.none)}
  in
...
let navigationProgram :
    ...
    -> 'flags
    -> 'msg debug_msg Tea_app.programInterface =
 fun ?ssr location_to_msg {init; update; view; subscriptions; shutdown}
  ...
  in
  Tea_navigation.navigationProgram ?ssr location debugged opnode flags
