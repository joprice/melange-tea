...
let render_selected = function
  | Some selected ->
      div []
        [ text ("you selected " ^ selected); div [ onClick Delete ] [ text "delete selection" ] ]
...
let view state =
  div [] [ render_selected state.selected; render_languages state.selected state.languages ]

let main =
  let initialState = { selected = Some "Erlang"; languages = [ "Erlang"; "Ocaml"; "Clojure" ] } in
